    ...
    try:
        ...
        for result in results.get("attack_results", []):
            dataset_data.append({
                ...
                "avg_proxy_completion_tokens": sum(result.get("proxy_completion_tokens", [])) / len(result.get("proxy_completion_tokens", [])) if result.get("proxy_completion_tokens") else 0.0,
                "avg_target_completion_tokens": sum(result.get("target_completion_tokens", [])) / len(result.get("target_completion_tokens", [])) if result.get("target_completion_tokens") else 0.0,
